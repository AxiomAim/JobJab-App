...
import {
    ...
    AxiomaimVerticalNavigationAppearance,
    AxiomaimVerticalNavigationMode,
    AxiomaimVerticalNavigationPosition,
} from '@axiomaim/components/navigation/navigation.types';
import { AxiomaimVerticalNavigationAsideItemComponent } from '@axiomaim/components/navigation/vertical/components/aside/aside.component';
...
import { AxiomaimVerticalNavigationGroupItemComponent } from '@axiomaim/components/navigation/vertical/components/group/group.component';
...
import { AxiomaimScrollbarDirective } from '@axiomaim/directives/scrollbar/scrollbar.directive';
...
@Component({
    selector: 'axiomaim-vertical-navigation',
    templateUrl: './vertical.component.html',
    styleUrls: ['./vertical.component.scss'],
    animations: axiomaimAnimations,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs: 'axiomaimVerticalNavigation',
    imports: [
        AxiomaimScrollbarDirective,
        AxiomaimVerticalNavigationAsideItemComponent,
        ...
        AxiomaimVerticalNavigationGroupItemComponent,
        AxiomaimVerticalNavigationSpacerItemComponent,
    ],
})
export class AxiomaimVerticalNavigationComponent
    implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: AxiomaimVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: AxiomaimVerticalNavigationMode = 'side';
    @Input() name: string = this._axiomaimUtilsService.randomId();
    @Input() navigation: AxiomaimNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: AxiomaimVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output()
    readonly appearanceChanged: EventEmitter<AxiomaimVerticalNavigationAppearance> =
        new EventEmitter<AxiomaimVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<AxiomaimVerticalNavigationMode> =
        new EventEmitter<AxiomaimVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> =
        new EventEmitter<boolean>();
    @Output()
    readonly positionChanged: EventEmitter<AxiomaimVerticalNavigationPosition> =
        new EventEmitter<AxiomaimVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<AxiomaimNavigationItem> =
        new ReplaySubject<AxiomaimNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<AxiomaimNavigationItem> =
        new ReplaySubject<AxiomaimNavigationItem>(1);
    ...
    private _axiomaimScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'axiomaim-vertical-navigation-animations-enabled':
                this._animationsEnabled,
            [`axiomaim-vertical-navigation-appearance-${this.appearance}`]: true,
            'axiomaim-vertical-navigation-hover': this._hovered,
            'axiomaim-vertical-navigation-inner': this.inner,
            'axiomaim-vertical-navigation-mode-over': this.mode === 'over',
            'axiomaim-vertical-navigation-mode-side': this.mode === 'side',
            'axiomaim-vertical-navigation-opened': this.opened,
            'axiomaim-vertical-navigation-position-left': this.position === 'left',
            'axiomaim-vertical-navigation-position-right':
                this.position === 'right',
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    /**
     * Setter for axiomaimScrollbarDirectives
     */
    @ViewChildren(AxiomaimScrollbarDirective)
    set axiomaimScrollbarDirectives(
        axiomaimScrollbarDirectives: QueryList<AxiomaimScrollbarDirective>
    ) {
        // Store the directives
        this._axiomaimScrollbarDirectives = axiomaimScrollbarDirectives;

        // Return if there are no directives
        if (axiomaimScrollbarDirectives.length === 0) {
            return;
        }

        // Unsubscribe the previous subscriptions
        if (this._axiomaimScrollbarDirectivesSubscription) {
            this._axiomaimScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._axiomaimScrollbarDirectivesSubscription = merge(
    ...
    ngAfterViewInit(): void {
        ...
        setTimeout(() => {
            ...
            if (
                ...
                const activeItem =
                    this._navigationContentEl.nativeElement.querySelector(
                        '.axiomaim-vertical-navigation-item-active'
                    );
            ...
            // Otherwise
                ...
                this._axiomaimScrollbarDirectives.forEach(
                    (axiomaimScrollbarDirective) => {
                        // Skip if not enabled
                        if (!axiomaimScrollbarDirective.isEnabled()) {
                            return;
                        }

                        // Scroll to the active element
                        axiomaimScrollbarDirective.scrollToElement(
                            '.axiomaim-vertical-navigation-item-active',
                            -120,
                            true
                        );
     ...
     * @param item
     */
    openAside(item: AxiomaimNavigationItem): void {
     ...
     * @param item
     */
    toggleAside(item: AxiomaimNavigationItem): void {
        // Toggle
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('axiomaim-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add(
                'axiomaim-vertical-navigation-overlay-transparent'
            );
        }
    ...
    private _showAsideOverlay(): void {
        ...
        this._asideOverlay.classList.add(
            'axiomaim-vertical-navigation-aside-overlay'
        );
