...
import {
    ScrollbarGeometry,
    ScrollbarPosition,
} from '@axiomaim/directives/scrollbar/scrollbar.types';
 ...
 */
@Directive({
    selector: '[axiomaimScrollbar]',
    exportAs: 'axiomaimScrollbar',
    standalone: true,
})
export class AxiomaimScrollbarDirective implements OnChanges, OnInit, OnDestroy {
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_axiomaimScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    private _elementRef = inject(ElementRef);
    private _platform = inject(Platform);

    @Input() axiomaimScrollbar: boolean = true;
    @Input() axiomaimScrollbarOptions: PerfectScrollbar.Options;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        // Enabled
        if ('axiomaimScrollbar' in changes) {
            // Interpret empty string as 'true'
            this.axiomaimScrollbar = coerceBooleanProperty(
                changes.axiomaimScrollbar.currentValue
            );

            // If enabled, init the directive
            if (this.axiomaimScrollbar) {
                this._init();
            }
        ...
        // Scrollbar options
        if ('axiomaimScrollbarOptions' in changes) {
            // Merge the options
            this._options = merge(
                {},
                this._options,
                changes.axiomaimScrollbarOptions.currentValue
            );
