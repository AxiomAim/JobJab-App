...
import { AXIOMAIM_MOCK_API_DEFAULT_DELAY } from '@axiomaim/lib/mock-api/mock-api.constants';
import { AxiomaimMockApiService } from '@axiomaim/lib/mock-api/mock-api.service';
...
export const mockApiInterceptor = (
    ...
    const defaultDelay = inject(AXIOMAIM_MOCK_API_DEFAULT_DELAY);
    const axiomaimMockApiService = inject(AxiomaimMockApiService);

    // Try to get the request handler
    const { handler, urlParams } = axiomaimMockApiService.findHandler(
