...
import {
    ...
    emails as emailsData,
    settings as settingsData,
} from 'app/mock-api/crm/emailbox/data';
import { assign, cloneDeep } from 'lodash-es';

@Injectable({ providedIn: 'root' })
export class EmailboxMockApi {
    private _filters: any[] = filtersData;
    private _folders: any[] = foldersData;
    private _emails: any[] = emailsData;
    ...
    registerHandlers(): void {
        ...
        this._axiomaimMockApiService
            .onGet('api/crm/emailbox/settings')
        ...
        // -----------------------------------------------------------------------------------------------------
        this._axiomaimMockApiService
            .onPatch('api/crm/emailbox/settings')
            .reply(({ request }) => {
                // Get the settings
        ...
        // @ Folders - GET
        // -----------------------------------------------------------------------------------------------------
        this._axiomaimMockApiService.onGet('api/crm/emailbox/folders').reply(() => {
            let count = 0;

            // Iterate through the folders
            this._folders.forEach((folder) => {
                // Get the mails of this folder
                const mails = this._emails.filter(
        ...
        // -----------------------------------------------------------------------------------------------------
        this._axiomaimMockApiService
            .onGet('api/crm/emailbox/filters')
        ...
        // -----------------------------------------------------------------------------------------------------
        this._axiomaimMockApiService
            .onPost('api/crm/emailbox/label')
            .reply(({ request }) => {
                // Get the label
                const label = cloneDeep(request.body.label);

                // Generate an id
                label.id = AxiomaimMockApiUtils.guid();
        ...
        // -----------------------------------------------------------------------------------------------------
        this._axiomaimMockApiService
            .onPatch('api/crm/emailbox/label')
            .reply(({ request }) => {
        ...
        // -----------------------------------------------------------------------------------------------------
        this._axiomaimMockApiService
            .onDelete('api/crm/emailbox/label')
            .reply(({ request }) => {
                // Get the id
                ...
                const mailsWithLabel = this._emails.filter(
        ...
        // -----------------------------------------------------------------------------------------------------
        this._axiomaimMockApiService
            .onGet('api/crm/emailbox/mails', 625)
            .reply(({ request }) => {
                ...
                let mails: any[] | null = cloneDeep(this._emails);
        ...
        // -----------------------------------------------------------------------------------------------------
        this._axiomaimMockApiService
            .onGet('api/crm/emailbox/mail')
            .reply(({ request }) => {
                // Get the id from the params
                const id = request.params.get('id');

                // Clone the mails mock-api to prevent accidental mock-api updates
                const mails = cloneDeep(this._emails);
        ...
        this._axiomaimMockApiService
            ...
            .reply(({ request }) => {
                ...
                this._emails.forEach((item, index, mails) => {
                    if (item.id === id) {
                        // Update the mail
