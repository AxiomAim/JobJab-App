...
export class RequestsDataService extends BaseDataService<Request> {
    ...
    public getAll(orgId: string): Observable<Request[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    public getItem(id: string): Observable<Request> {
    ...
    public updateItem(data: Partial<Request>): Observable<Request> {
      console.log("updateItem:data", data);
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as Request); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<Request>(this.baseCollection, id, updateData);
      }
    ...
    public createItem(data: Request): Observable<Request> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<Request[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<Request>[]): Observable<Request[]> { 
      return this._firestore.bulkCreate<Request>(this.baseCollection, data as Request[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<Request[]> {
      return this._firestore.getQueryWhereclause<Request>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<Request>[]): Observable<Request[]> { 
      return this._firestore.bulkUpdate<Request>(this.baseCollection, data as Request[]); 
    }
   ...
   */
    public getPaged(
      pageSize: number,
      startAfterDoc?: Request,
      orderByField?: string,
      orderByDirection: 'asc' | 'desc' = 'asc'
    ): Observable<{ data: Request[]; lastDoc: Request | null }> {
      return new Observable<{ data: Request[]; lastDoc: Request | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: Request[] = [];
            let lastDoc: Request | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as Request);
              lastDoc = doc.data() as Request; // Get the last document for next page
            });
