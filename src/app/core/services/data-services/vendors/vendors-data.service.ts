...
export class VendorsDataService extends BaseDataService<Vendor> {
    ...
    public getAll(orgId: string): Observable<Vendor[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    public getItem(id: string): Observable<Vendor> {
    ...
    public updateItem(data: Partial<Vendor>): Observable<Vendor> {
      console.log("updateItem:data", data);
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as Vendor); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<Vendor>(this.baseCollection, id, updateData);
      }
    ...
    public createItem(data: Vendor): Observable<Vendor> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<Vendor[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<Vendor>[]): Observable<Vendor[]> { 
      return this._firestore.bulkCreate<Vendor>(this.baseCollection, data as Vendor[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<Vendor[]> {
      return this._firestore.getQueryWhereclause<Vendor>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<Vendor>[]): Observable<Vendor[]> { 
      return this._firestore.bulkUpdate<Vendor>(this.baseCollection, data as Vendor[]); 
    }
   ...
   */
    public getPaged(
      pageSize: number,
      startAfterDoc?: Vendor,
      orderByField?: string,
      orderByDirection: 'asc' | 'desc' = 'asc'
    ): Observable<{ data: Vendor[]; lastDoc: Vendor | null }> {
      return new Observable<{ data: Vendor[]; lastDoc: Vendor | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: Vendor[] = [];
            let lastDoc: Vendor | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as Vendor);
              lastDoc = doc.data() as Vendor; // Get the last document for next page
            });
