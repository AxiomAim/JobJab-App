...
export class DealsDataService extends BaseDataService<Deal> {
    ...
    public getAll(orgId: string): Observable<Deal[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    public getItem(id: string): Observable<Deal> {
    ...
    public updateItem(data: Partial<Deal>): Observable<Deal> {
      console.log("updateItem:data", data);
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as Deal); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<Deal>(this.baseCollection, id, updateData);
      }
    ...
    public createItem(data: Deal): Observable<Deal> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<Deal[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<Deal>[]): Observable<Deal[]> { 
      return this._firestore.bulkCreate<Deal>(this.baseCollection, data as Deal[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<Deal[]> {
      return this._firestore.getQueryWhereclause<Deal>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<Deal>[]): Observable<Deal[]> { 
      return this._firestore.bulkUpdate<Deal>(this.baseCollection, data as Deal[]); 
    }
   ...
   */
    ...
    ): Observable<{ data: Deal[]; lastDoc: Deal | null }> {
      return new Observable<{ data: Deal[]; lastDoc: Deal | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: Deal[] = [];
            let lastDoc: Deal | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as Deal);
              lastDoc = doc.data() as Deal; // Get the last document for next page
            });
