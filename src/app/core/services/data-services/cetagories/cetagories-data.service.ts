...
export class CategoriesDataService extends BaseDataService<Category> {
    ...
    public getAll(orgId: string): Observable<Category[]> {
    ...
    public updateItem(data: Partial<Category>): Observable<Category> {
      console.log("updateItem:data", data);
        ...
        if (Object.keys(updateData).length === 0) {
          return of(data as Category); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<Category>(this.baseCollection, id, updateData);
      }
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<Category[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<Category>[]): Observable<Category[]> { 
      return this._firestore.bulkCreate<Category>(this.baseCollection, data as Category[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<Category[]> {
      return this._firestore.getQueryWhereclause<Category>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<Category>[]): Observable<Category[]> { 
      return this._firestore.bulkUpdate<Category>(this.baseCollection, data as Category[]); 
    }
   ...
   */
    public getPaged(
      pageSize: number,
      startAfterDoc?: Category,
      orderByField?: string,
      orderByDirection: 'asc' | 'desc' = 'asc'
    ): Observable<{ data: Category[]; lastDoc: Category | null }> {
      return new Observable<{ data: Category[]; lastDoc: Category | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: Category[] = [];
            let lastDoc: Category | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as Category);
              lastDoc = doc.data() as Category; // Get the last document for next page
            });
