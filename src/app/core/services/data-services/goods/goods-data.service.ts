...
export class GoodsDataService extends BaseDataService<Good> {
    ...
    public getAll(orgId: string): Observable<Good[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    public getItem(id: string): Observable<Good> {
    ...
    public updateItem(data: Partial<Good>): Observable<Good> {
      console.log("updateItem:data", data);
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as Good); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<Good>(this.baseCollection, id, updateData);
      }
    ...
    public createItem(data: Good): Observable<Good> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<Good[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<Good>[]): Observable<Good[]> { 
      return this._firestore.bulkCreate<Good>(this.baseCollection, data as Good[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<Good[]> {
      return this._firestore.getQueryWhereclause<Good>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<Good>[]): Observable<Good[]> { 
      return this._firestore.bulkUpdate<Good>(this.baseCollection, data as Good[]); 
    }
   ...
   */
    ...
    ): Observable<{ data: Good[]; lastDoc: Good | null }> {
      return new Observable<{ data: Good[]; lastDoc: Good | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: Good[] = [];
            let lastDoc: Good | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as Good);
              lastDoc = doc.data() as Good; // Get the last document for next page
            });
