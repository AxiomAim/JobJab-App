...
export class UserAppointmentsDataService extends BaseDataService<UserAppointment> {
    ...
    constructor(
      public firestore: Firestore
        ...
        super('user-appointments');
    }
    
    public getAll(orgId: string): Observable<UserAppointment[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    public getItem(id: string): Observable<UserAppointment> {
    ...
    public updateItem(data: Partial<UserAppointment>): Observable<UserAppointment> {
      console.log("updateItem:data", data);
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as UserAppointment); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<UserAppointment>(this.baseCollection, id, updateData);
      }
    ...
    public createItem(data: UserAppointment): Observable<UserAppointment> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<UserAppointment[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<UserAppointment>[]): Observable<UserAppointment[]> { 
      return this._firestore.bulkCreate<UserAppointment>(this.baseCollection, data as UserAppointment[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<UserAppointment[]> {
      return this._firestore.getQueryWhereclause<UserAppointment>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<UserAppointment>[]): Observable<UserAppointment[]> { 
      return this._firestore.bulkUpdate<UserAppointment>(this.baseCollection, data as UserAppointment[]); 
    }
   ...
   */
    public getPaged(
      pageSize: number,
      startAfterDoc?: UserAppointment,
      orderByField?: string,
      orderByDirection: 'asc' | 'desc' = 'asc'
    ): Observable<{ data: UserAppointment[]; lastDoc: UserAppointment | null }> {
      return new Observable<{ data: UserAppointment[]; lastDoc: UserAppointment | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: UserAppointment[] = [];
            let lastDoc: UserAppointment | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as UserAppointment);
              lastDoc = doc.data() as UserAppointment; // Get the last document for next page
            });
