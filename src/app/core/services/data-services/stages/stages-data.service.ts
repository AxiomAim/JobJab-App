...
export class StagesDataService extends BaseDataService<Stage> {
    ...
    public getAll(orgId: string): Observable<Stage[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    public getItem(id: string): Observable<Stage> {
    ...
    public updateItem(data: Partial<Stage>): Observable<Stage> {
      console.log("updateItem:data", data);
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as Stage); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<Stage>(this.baseCollection, id, updateData);
      }
    ...
    public createItem(data: Stage): Observable<Stage> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<Stage[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<Stage>[]): Observable<Stage[]> { 
      return this._firestore.bulkCreate<Stage>(this.baseCollection, data as Stage[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<Stage[]> {
      return this._firestore.getQueryWhereclause<Stage>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<Stage>[]): Observable<Stage[]> { 
      return this._firestore.bulkUpdate<Stage>(this.baseCollection, data as Stage[]); 
    }
   ...
   */
    ...
    ): Observable<{ data: Stage[]; lastDoc: Stage | null }> {
      return new Observable<{ data: Stage[]; lastDoc: Stage | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: Stage[] = [];
            let lastDoc: Stage | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as Stage);
              lastDoc = doc.data() as Stage; // Get the last document for next page
            });
