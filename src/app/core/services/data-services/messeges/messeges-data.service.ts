...
export class MessagesDataService extends BaseDataService<Message> {
    ...
    public getAll(orgId: string): Observable<Message[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    public getItem(id: string): Observable<Message> {
    ...
    public updateItem(data: Partial<Message>): Observable<Message> {
      console.log("updateItem:data", data);
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as Message); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<Message>(this.baseCollection, id, updateData);
      }
    ...
    public deleteItem(id: string): Observable<Message> {
    ...
    public createItem(data: Message): Observable<Message> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<Message[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<Message>[]): Observable<Message[]> { 
      return this._firestore.bulkCreate<Message>(this.baseCollection, data as Message[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<Message[]> {
      return this._firestore.getQueryWhereclause<Message>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<Message>[]): Observable<Message[]> { 
      return this._firestore.bulkUpdate<Message>(this.baseCollection, data as Message[]); 
    }
   ...
   */
    public getPaged(
      pageSize: number,
      startAfterDoc?: Message,
      orderByField?: string,
      orderByDirection: 'asc' | 'desc' = 'asc'
    ): Observable<{ data: Message[]; lastDoc: Message | null }> {
      return new Observable<{ data: Message[]; lastDoc: Message | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: Message[] = [];
            let lastDoc: Message | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as Message);
              lastDoc = doc.data() as Message; // Get the last document for next page
            });
