...
export const FirebaseAuthV2Service = createInjectable(() => {
  ...
  const signIn = async (credentials: { email: string; password: string }): Promise<User> => {
    await signInWithEmailAndPassword(auth, credentials.email, credentials.password).then(async(cred) => {  
      console.log('signIn cred', cred);
      authUser.set(cred.user);
      const thisUser = await firstValueFrom(_usersDataService.getItem(cred.user.uid));
  ...
  const signIn1 = (credentials: { email: string; password: string }): Observable<User> => {
    ...
    return from(signInWithEmailAndPassword(auth, credentials.email, credentials.password)).pipe(
      tap(cred => authUser.set(cred)),
      switchMap(cred => _usersDataService.getItem(cred.user.uid).pipe(
        tap(thisUser => {
          const encodedData = btoa(credentials.password);
          thisUser.emailKey = encodedData;
        }),
        switchMap(thisUser => _usersDataService.updateItem(thisUser).pipe(
          tap(updateUser => {
          ...
          switchMap(() => _organizationsDataService.getItem(domain).pipe(
            tap(thisOrganization => {
              organization.set(thisOrganization);
              setToStorage();
            }),
