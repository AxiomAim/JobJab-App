...
@Component({
    selector: 'quotes-requests-details',
...
export class QuotesRequestsDetailsComponent implements OnInit, OnDestroy {
    ...
    private _quotesRequest: BehaviorSubject<QuotesRequest | null> = new BehaviorSubject(
        null
    );
    get quote$(): Observable<QuotesRequest> {
        return this._quotesRequest.asObservable();
    }
    ...
    constructor(
        ...
        private _quotesListComponent: QuotesRequestsListComponent,
     ...
     */
    ngOnInit(): void {
        this.quotesRequests = this._quotesRequestsV2Service.quotesRequests();
        this.quotesRequest = this._quotesRequestsV2Service.quoteRequest();
        // Open the drawer
        ...
        this._quotesListComponent.matDrawer.open();
        this.quotesRequestForm.patchValue(this.quotesRequest);
     ...
     */
    updateItem(): void {        
        this.quotesRequest = {...this._quotesRequest.getValue(), ...this.quotesRequestForm.getRawValue()};
        console.log('quote', this.quotesRequest);
        // Get the quote object
        // const quote = this.quoteForm.getRawValue();

        // Update the quote on the server
        this._quotesRequestsV2Service
            .updateItem(this.quotesRequest)
            .then(() => {
    ...
    deleteProduct(): void {
        ...
        confirmation.afterClosed().subscribe((result) => {
            ...
            if (result === 'confirmed') {
                ...
                const id = this.quotesRequest.id;

                // Get the next/previous quote's id
                const currentProductIndex = this.quotesRequests.findIndex(
                    (item) => item.id === id
                );
                const nextProductIndex =
                    currentProductIndex +
                    (currentProductIndex === this.quotesRequests.length - 1 ? -1 : 1);
                const nextProductId =
                    this.quotesRequests.length === 1 && this.quotesRequests[0].id === id
                        ? null
                        : this.quotesRequests[nextProductIndex].id;
    ...
    removeAvatar(): void {
        ...
        const avatarFormControl = this.quotesRequestForm.get('avatar');
    ...
    onOptionSelected(data: any[]) {
        ...
        this._quotesRequest.next(this.quotesRequest);
