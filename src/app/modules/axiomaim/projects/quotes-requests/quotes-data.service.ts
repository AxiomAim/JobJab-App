...
export class QuotesDataService extends BaseDataService<QuotesRequest> {
    ...
    constructor(
      public firestore: Firestore
        ...
        super('quotes-requests');
    }
    
    public getAll(orgId: string): Observable<QuotesRequest[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    public getItem(id: string): Observable<QuotesRequest> {
    ...
    public updateItem(data: Partial<QuotesRequest>): Observable<QuotesRequest> {
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as QuotesRequest); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<QuotesRequest>(this.baseCollection, id, updateData);
      }
    ...
    public createItem(data: QuotesRequest): Observable<QuotesRequest> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<QuotesRequest[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<QuotesRequest>[]): Observable<QuotesRequest[]> { 
      return this._firestore.bulkCreate<QuotesRequest>(this.baseCollection, data as QuotesRequest[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<QuotesRequest[]> {
      return this._firestore.getQueryWhereclause<QuotesRequest>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<QuotesRequest>[]): Observable<QuotesRequest[]> { 
      return this._firestore.bulkUpdate<QuotesRequest>(this.baseCollection, data as QuotesRequest[]); 
    }
   ...
   */
    public getPaged(
      pageSize: number,
      startAfterDoc?: QuotesRequest,
      orderByField?: string,
      orderByDirection: 'asc' | 'desc' = 'asc'
    ): Observable<{ data: QuotesRequest[]; lastDoc: QuotesRequest | null }> {
      return new Observable<{ data: QuotesRequest[]; lastDoc: QuotesRequest | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: QuotesRequest[] = [];
            let lastDoc: QuotesRequest | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as QuotesRequest);
              lastDoc = doc.data() as QuotesRequest; // Get the last document for next page
            });
