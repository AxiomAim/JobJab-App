...
export class QuotesDataService extends BaseDataService<Quote> {
    ...
    public updateItem(data: Partial<Quote>): Observable<Quote> {
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as Quote); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<Quote>(this.baseCollection, id, updateData);
      }
    ...
    public deleteItem(id: string): Observable<Quote> {
    ...
    public createItem(data: Quote): Observable<Quote> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<Quote[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<Quote>[]): Observable<Quote[]> { 
      return this._firestore.bulkCreate<Quote>(this.baseCollection, data as Quote[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<Quote[]> {
      return this._firestore.getQueryWhereclause<Quote>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<Quote>[]): Observable<Quote[]> { 
      return this._firestore.bulkUpdate<Quote>(this.baseCollection, data as Quote[]); 
    }
   ...
   */
    ...
    ): Observable<{ data: Quote[]; lastDoc: Quote | null }> {
      return new Observable<{ data: Quote[]; lastDoc: Quote | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: Quote[] = [];
            let lastDoc: Quote | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as Quote);
              lastDoc = doc.data() as Quote; // Get the last document for next page
            });
