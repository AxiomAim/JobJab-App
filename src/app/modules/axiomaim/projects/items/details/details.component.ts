...
export class ItemsDetailsComponent implements OnInit, OnDestroy {
    ...
    private _item: BehaviorSubject<Item | null> = new BehaviorSubject(
        null
    );
    ...
    constructor(
        ...
        private _itemsListComponent: ItemsListComponent,
     ...
     */
    ngOnInit(): void {
        this.items = this._itemsV2Service.items();
        this.item = this._itemsV2Service.item();
        // Open the drawer
     ...
     */
    updateItem(): void {        
        this.item = {...this._item.getValue(), ...this.itemForm.getRawValue()};
        console.log('item', this.item);
        // Get the item object
        // const item = this.itemForm.getRawValue();

        // Update the item on the server
        this._itemsV2Service
    ...
    deleteProduct(): void {
        ...
        confirmation.afterClosed().subscribe((result) => {
            ...
            if (result === 'confirmed') {
                ...
                const currentProductIndex = this.items.findIndex(
                    (item) => item.id === id
                );
                const nextProductIndex =
                    currentProductIndex +
                    (currentProductIndex === this.items.length - 1 ? -1 : 1);
                const nextProductId =
                    this.items.length === 1 && this.items[0].id === id
                        ? null
                        : this.items[nextProductIndex].id;
    ...
    uploadAvatar(event: any): void {
        ...
        // this._itemsV2Service.uploadAvatar(event.target.files[0], 'items').subscribe({
        //     next: (response: any) => {
        //         this._axiomaimLoadingService.hide();
        //         console.log('uploadAvatar', response)
        //         this.item.avatarPath = response.filePath;
        //         this.item.avatarFile = response.fileName;
        //         this.item.avatarType = response.fileType;
        //         this.item.avatarUrl = response.fileUrl;
        //         this.item.avatar = response.fileUrl;                
    ...
    onOptionSelected(data: any[]) {
        ...
        // this.item.itemRoles = data;
        this._item.next(this.item);
        // console.log('onOptionSelected', this.item);
        // this.item$.subscribe((resProduct: Product) => {
    ...
    //     const isTagApplied = this.item.tags.find((id) => id === tag.id);

    //     // If the found tag is already applied to the item...
    //     if (isTagApplied) {
    //         // Remove the tag from the item
    //         this.removeTagFromProduct(tag);
    //     } else {
    //         // Otherwise add the tag to the item
