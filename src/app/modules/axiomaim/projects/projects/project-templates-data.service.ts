...
import { TBoard } from './project-template.model';
...
export class ProtocolTemplatesDataService extends BaseDataService<TBoard> {
    ...
    constructor(
      public firestore: Firestore
        ...
        super('aim-protocol-templates');
    }
    
    public getAll(): Observable<TBoard[]> {
        return  this._firestore.getAll(this.baseCollection);
    }
    public getItem(id: string): Observable<TBoard> {
    ...
    public updateItem(data: Partial<TBoard>): Observable<TBoard> {
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as TBoard); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<TBoard>(this.baseCollection, id, updateData);
      }
    ...
    public createItem(data: TBoard): Observable<TBoard> {
        return this._firestore.createItem<TBoard>(this.baseCollection, data).pipe(
          take(1),
          tap((createdItem) => { 
          ...
          catchError((error) => {
            ...
            return of(null as TBoard); // Or: throw error; 
          })
        );
    }

    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<TBoard[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<TBoard>[]): Observable<TBoard[]> { 
      return this._firestore.bulkCreate<TBoard>(this.baseCollection, data as TBoard[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<TBoard[]> {
      return this._firestore.getQueryWhereclause<TBoard>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<TBoard>[]): Observable<TBoard[]> { 
      return this._firestore.bulkUpdate<TBoard>(this.baseCollection, data as TBoard[]); 
    }
   ...
   */
    public getPaged(
      pageSize: number,
      startAfterDoc?: TBoard,
      orderByField?: string,
      orderByDirection: 'asc' | 'desc' = 'asc'
    ): Observable<{ data: TBoard[]; lastDoc: TBoard | null }> {
      return new Observable<{ data: TBoard[]; lastDoc: TBoard | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: TBoard[] = [];
            let lastDoc: TBoard | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as TBoard);
              lastDoc = doc.data() as TBoard; // Get the last document for next page
            });
