...
export class ProjectLabelsDataService extends BaseDataService<ProjectLabel> {
    ...
    constructor(
      public firestore: Firestore
        ...
        super('aim-project-labels');
    }
    
    public getAll(): Observable<ProjectLabel[]> {
        return  this._firestore.getAll(this.baseCollection);
    }
    public getItem(id: string): Observable<ProjectLabel> {
    ...
    public updateItem(data: Partial<ProjectLabel>): Observable<ProjectLabel> {
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as ProjectLabel); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<ProjectLabel>(this.baseCollection, id, updateData);
      }
      ...
      return this._firestore.getItem(this.baseCollection, id).pipe(
        ...
        switchMap((document) => {
          if (document) { 
            ...
            throw new Error(`ProjectLabel with ID ${id} not found`); 
          }
        })
      );
    }
    ...
    public createItem(data: ProjectLabel): Observable<ProjectLabel> {
        return this._firestore.createItem(this.baseCollection, data).pipe(take(1),
          tap((createdProjectLabel) => { 
            console.log('Item created successfully:', createdProjectLabel); 
          }),
          catchError((error) => {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<ProjectLabel[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<ProjectLabel>[]): Observable<ProjectLabel[]> { 
      return this._firestore.bulkCreate<ProjectLabel>(this.baseCollection, data as ProjectLabel[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<ProjectLabel[]> {
      return this._firestore.getQueryWhereclause<ProjectLabel>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<ProjectLabel>[]): Observable<ProjectLabel[]> { 
      return this._firestore.bulkUpdate<ProjectLabel>(this.baseCollection, data as ProjectLabel[]); 
    }
   ...
   */
    public getPaged(
      pageSize: number,
      startAfterDoc?: ProjectLabel,
      orderByField?: string,
      orderByDirection: 'asc' | 'desc' = 'asc'
    ): Observable<{ data: ProjectLabel[]; lastDoc: ProjectLabel | null }> {
      return new Observable<{ data: ProjectLabel[]; lastDoc: ProjectLabel | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: ProjectLabel[] = [];
            let lastDoc: ProjectLabel | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as ProjectLabel);
              lastDoc = doc.data() as ProjectLabel; // Get the last document for next page
            });
