...
export class ServicesAddServiceComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    async onSubmit() {
        ...
        let item: ServiceOffering = ServiceOfferingModel.emptyDto();
        item.title = this.serviceForm.value.title;
        item.description = this.serviceForm.value.description;
        await this._serviceOfferingsV2Service.createItem(item).then(async (serviceOffering) => {
