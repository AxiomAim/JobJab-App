...
export class QueriesAddItemComponent implements OnInit, AfterViewInit, OnDestroy {
    @Input() btnIcon: string = 'mat_outline:add';
    @Input() btnTitle: string = 'Add';
    @Input() external: boolean = false;
    @Output() queryCreated: EventEmitter<Query> = new EventEmitter<Query>();
    ...
    public newContact: Query = QueryModel.emptyDto();
    ...
    setupForm() {
        this.newContact = QueryModel.emptyDto();
    ...
    private resetForm(): void {
        ...
        this.updateFormData(QueryModel.emptyDto());
    ...
    async onSubmit() {
        ...
        try {
            ...
            await this._queriesV2Service.createItem(this.newContact);
            await this._queriesV2Service.getAll();
            ...
            if(this.external){
                ...
                this.close();
                // this._router.navigate(['/crm/queries']);
            }
        } finally {
    ...
    sendContact() {
        this.queryCreated.emit(this.newContact);
        this.close();
    }
