...
export class FormsDataService extends BaseDataService<Form> {
    ...
    public getAll(orgId: string): Observable<Form[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    public getItem(id: string): Observable<Form> {
    ...
    public updateItem(data: Partial<Form>): Observable<Form> {
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as Form); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<Form>(this.baseCollection, id, updateData);
      }
    ...
    public deleteItem(id: string): Observable<Form> {
    ...
    public createItem(data: Form): Observable<Form> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<Form[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<Form>[]): Observable<Form[]> { 
      return this._firestore.bulkCreate<Form>(this.baseCollection, data as Form[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<Form[]> {
      return this._firestore.getQueryWhereclause<Form>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<Form>[]): Observable<Form[]> { 
      return this._firestore.bulkUpdate<Form>(this.baseCollection, data as Form[]); 
    }
   ...
   */
    ...
    ): Observable<{ data: Form[]; lastDoc: Form | null }> {
      return new Observable<{ data: Form[]; lastDoc: Form | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: Form[] = [];
            let lastDoc: Form | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as Form);
              lastDoc = doc.data() as Form; // Get the last document for next page
            });
