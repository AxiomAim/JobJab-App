...
export class ServicesDataService extends BaseDataService<Item> {
    ...
    public getAll(orgId: string): Observable<Item[]> {
    ...
    public bulkCreate(data: Partial<Item>[]): Observable<Item[]> { 
      return this._firestore.bulkCreate<Item>(this.baseCollection, data as Item[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<Item[]> {
      return this._firestore.getQueryWhereclause<Item>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<Item>[]): Observable<Item[]> { 
      return this._firestore.bulkUpdate<Item>(this.baseCollection, data as Item[]); 
    }
   ...
   */
    ...
    ): Observable<{ data: Item[]; lastDoc: Item | null }> {
      return new Observable<{ data: Item[]; lastDoc: Item | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: Item[] = [];
            let lastDoc: Item | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as Item);
              lastDoc = doc.data() as Item; // Get the last document for next page
            });
