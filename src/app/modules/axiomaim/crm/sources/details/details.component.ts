...
import { SourcesListComponent } from '../list/list.component';
...
export class SourcesDetailsComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        private _sourcesListComponent: SourcesListComponent,
     ...
     */
    ngOnInit(): void {
        this.sources = this._sourcesV2Service.sources();
        this.source = this._sourcesV2Service.source();
        // Open the drawer
        this._sourcesListComponent.matDrawer.open();
    ...
    closeDrawer(): Promise<MatDrawerToggleResult> {
        return this._sourcesListComponent.matDrawer.close();
    }
     ...
     */
    updateItem(): void {        
        this.source = {...this._source.getValue(), ...this.sourceForm.getRawValue()};
        console.log('source', this.source);
        // Get the source object
        // const source = this.sourceForm.getRawValue();

        // Update the source on the server
        this._sourcesV2Service
            .updateItem(this.source)
            .then(() => {
    ...
    deleteProduct(): void {
        ...
        confirmation.afterClosed().subscribe((result) => {
            ...
            if (result === 'confirmed') {
                // Get the current source's id
                const id = this.source.id;

                // Get the next/previous source's id
                const currentProductIndex = this.sources.findIndex(
                    (item) => item.id === id
                );
                const nextProductIndex =
                    currentProductIndex +
                    (currentProductIndex === this.sources.length - 1 ? -1 : 1);
                const nextProductId =
                    this.sources.length === 1 && this.sources[0].id === id
                        ? null
                        : this.sources[nextProductIndex].id;

                // Delete the source
                this._sourcesV2Service
                    .deleteItem(id)
                    .then((isDeleted) => {
                        // Return if the source wasn't deleted...
                        if (!isDeleted) {
                            return;
                        }

                        // Navigate to the next source if available
                        if (nextProductId) {
    ...
    uploadAvatar(event: any): void {
        ...
        // this._sourcesV2Service.uploadAvatar(event.target.files[0], 'sources').subscribe({
        //     next: (response: any) => {
        //         this._axiomaimLoadingService.hide();
        //         console.log('uploadAvatar', response)
        //         this.source.avatarPath = response.filePath;
        //         this.source.avatarFile = response.fileName;
        //         this.source.avatarType = response.fileType;
        //         this.source.avatarUrl = response.fileUrl;
        //         this.source.avatar = response.fileUrl;                
        //         this._source.next(this.source);
        //         console.log('source', this.source);
        //     },
        //     error: (error: any) => {
        //         this._axiomaimLoadingService.hide();
    ...
    removeAvatar(): void {
        ...
        const avatarFormControl = this.sourceForm.get('avatar');
        ...
        // Update the source
        // this.source.avatar = null;
    }

    onOptionSelected(data: any[]) {
        console.log('onOptionSelected', data);
        // this.source.sourceRoles = data;
        this._source.next(this.source);
        // console.log('onOptionSelected', this.source);
        // this.source$.subscribe((resProduct: Product) => {
    ...
    //     const isTagApplied = this.source.tags.find((id) => id === tag.id);
