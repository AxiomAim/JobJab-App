...
export class SourcesDataService extends BaseDataService<Source> {
    ...
    public getAll(orgId: string): Observable<Source[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    public getItem(id: string): Observable<Source> {
    ...
    public updateItem(data: Partial<Source>): Observable<Source> {
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as Source); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<Source>(this.baseCollection, id, updateData);
      }
    ...
    public createItem(data: Source): Observable<Source> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<Source[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<Source>[]): Observable<Source[]> { 
      return this._firestore.bulkCreate<Source>(this.baseCollection, data as Source[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<Source[]> {
      return this._firestore.getQueryWhereclause<Source>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<Source>[]): Observable<Source[]> { 
      return this._firestore.bulkUpdate<Source>(this.baseCollection, data as Source[]); 
    }
   ...
   */
    public getPaged(
      pageSize: number,
      startAfterDoc?: Source,
      orderByField?: string,
      orderByDirection: 'asc' | 'desc' = 'asc'
    ): Observable<{ data: Source[]; lastDoc: Source | null }> {
      return new Observable<{ data: Source[]; lastDoc: Source | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: Source[] = [];
            let lastDoc: Source | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as Source);
              lastDoc = doc.data() as Source; // Get the last document for next page
            });
