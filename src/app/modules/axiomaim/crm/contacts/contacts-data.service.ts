...
export class ContactsDataService extends BaseDataService<Contact> {
    ...
    public getAll(orgId: string): Observable<Contact[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    public getItem(id: string): Observable<Contact> {
    ...
    public updateItem(data: Partial<Contact>): Observable<Contact> {
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as Contact); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<Contact>(this.baseCollection, id, updateData);
      }
    ...
    public createItem(data: Contact): Observable<Contact> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<Contact[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<Contact>[]): Observable<Contact[]> { 
      return this._firestore.bulkCreate<Contact>(this.baseCollection, data as Contact[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<Contact[]> {
      return this._firestore.getQueryWhereclause<Contact>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<Contact>[]): Observable<Contact[]> { 
      return this._firestore.bulkUpdate<Contact>(this.baseCollection, data as Contact[]); 
    }
   ...
   */
    public getPaged(
      pageSize: number,
      startAfterDoc?: Contact,
      orderByField?: string,
      orderByDirection: 'asc' | 'desc' = 'asc'
    ): Observable<{ data: Contact[]; lastDoc: Contact | null }> {
      return new Observable<{ data: Contact[]; lastDoc: Contact | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: Contact[] = [];
            let lastDoc: Contact | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as Contact);
              lastDoc = doc.data() as Contact; // Get the last document for next page
            });
