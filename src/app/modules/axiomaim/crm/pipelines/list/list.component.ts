...
import { PipelinesAddItemComponent } from '../add-item/add-item.component';
...
@Component({
    ...
    imports: [
        ...
        AsyncPipe,
        I18nPluralPipe,
        PipelinesAddItemComponent,
        PhonePipe
    ],
})
export class PipelinesListComponent implements OnInit, OnDestroy {
    _pipelinesV2Service = inject(PipelinesV2Service);
    @ViewChild('matDrawer', { static: true }) matDrawer: MatDrawer;

    private _pipelines: BehaviorSubject<Pipeline[] | null> = new BehaviorSubject(
        null
    );
    ...
    ngOnInit(): void {
        // Get the quotes
        this._pipelines.next(this._pipelinesV2Service.pipelines());
        this.pipelines$
            .pipe(takeUntil(this._unsubscribeAll))
            .subscribe((quotes: Pipeline[]) => {
                // Update the counts
                this.pipelineCount = quotes.length;
        ...
        // Get the product
        this._pipeline.next(this._pipelinesV2Service.pipeline());
        this.pipeline$
            .pipe(takeUntil(this._unsubscribeAll))
            .subscribe((product: Pipeline) => {
        ...
        this.searchInputControl.valueChanges
            .pipe(
                ...
                switchMap((query) =>
                    // Search
                    this._pipelinesV2Service.search(query)
                )
            )
            .subscribe((resProducts) => {
                this._pipelines.next(resProducts);
            });
