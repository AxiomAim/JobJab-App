...
export class LeadsDataService extends BaseDataService<Lead> {
    ...
    public getAll(orgId: string): Observable<Lead[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    public getItem(id: string): Observable<Lead> {
    ...
    public updateItem(data: Partial<Lead>): Observable<Lead> {
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as Lead); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<Lead>(this.baseCollection, id, updateData);
      }
    ...
    public createItem(data: Lead): Observable<Lead> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<Lead[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<Lead>[]): Observable<Lead[]> { 
      return this._firestore.bulkCreate<Lead>(this.baseCollection, data as Lead[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<Lead[]> {
      return this._firestore.getQueryWhereclause<Lead>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<Lead>[]): Observable<Lead[]> { 
      return this._firestore.bulkUpdate<Lead>(this.baseCollection, data as Lead[]); 
    }
   ...
   */
    ...
    ): Observable<{ data: Lead[]; lastDoc: Lead | null }> {
      return new Observable<{ data: Lead[]; lastDoc: Lead | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: Lead[] = [];
            let lastDoc: Lead | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as Lead);
              lastDoc = doc.data() as Lead; // Get the last document for next page
            });
