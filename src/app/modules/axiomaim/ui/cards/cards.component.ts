...
export class CardsComponent implements AfterViewInit {
    @ViewChildren(AxiomaimCardComponent, { read: ElementRef })
    private _axiomaimCards: QueryList<ElementRef>;
    ...
    private _calcNumberOfCards(): void {
        ...
        this.filters.forEach((filter) => {
            ...
            if (filter === 'all') {
                count = this._axiomaimCards.length;
            } else {
                count = this.numberOfCards[filter] = this._axiomaimCards.filter(
                    (axiomaimCard) =>
                        axiomaimCard.nativeElement.classList.contains(
                            'filter-' + filter
                        )
                ).length;
            }
    ...
    private _filterCards(): void {
        // Go through all axiomaim-cards
        this._axiomaimCards.forEach((axiomaimCard) => {
            // If the 'all' filter is selected...
            if (this.selectedFilter === 'all') {
                // Remove hidden class from all cards
                axiomaimCard.nativeElement.classList.remove('hidden');
            }
            // Otherwise...
            else {
                ...
                // Otherwise
                    ...
                    axiomaimCard.nativeElement.classList.add('hidden');
                }
            }
        });
    }
