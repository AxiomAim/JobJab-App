...
export class TechniciansDataService extends BaseDataService<Technician> {
    ...
    public getAll(orgId: string): Observable<Technician[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    public getItem(id: string): Observable<Technician> {
    ...
    public updateItem(data: Partial<Technician>): Observable<Technician> {
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as Technician); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<Technician>(this.baseCollection, id, updateData);
      }
    ...
    public createItem(data: Technician): Observable<Technician> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<Technician[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<Technician>[]): Observable<Technician[]> { 
      return this._firestore.bulkCreate<Technician>(this.baseCollection, data as Technician[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<Technician[]> {
      return this._firestore.getQueryWhereclause<Technician>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<Technician>[]): Observable<Technician[]> { 
      return this._firestore.bulkUpdate<Technician>(this.baseCollection, data as Technician[]); 
    }
   ...
   */
    public getPaged(
      pageSize: number,
      startAfterDoc?: Technician,
      orderByField?: string,
      orderByDirection: 'asc' | 'desc' = 'asc'
    ): Observable<{ data: Technician[]; lastDoc: Technician | null }> {
      return new Observable<{ data: Technician[]; lastDoc: Technician | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            ...
            let lastDoc: Technician | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as Technician);
              lastDoc = doc.data() as Technician; // Get the last document for next page
            });
