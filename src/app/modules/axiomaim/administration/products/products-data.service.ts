...
export class ProductsDataService extends BaseDataService<Product> {
    ...
    public getAll(orgId: string): Observable<Product[]> {
    ...
    public updateItem(data: Partial<Product>): Observable<Product> {
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as Product); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<Product>(this.baseCollection, id, updateData);
      }
    ...
    public deleteItem(id: string): Observable<Product> {
    ...
    public createItem(data: Product): Observable<Product> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<Product[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<Product>[]): Observable<Product[]> { 
      return this._firestore.bulkCreate<Product>(this.baseCollection, data as Product[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<Product[]> {
      return this._firestore.getQueryWhereclause<Product>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<Product>[]): Observable<Product[]> { 
      return this._firestore.bulkUpdate<Product>(this.baseCollection, data as Product[]); 
    }
   ...
   */
    public getPaged(
      pageSize: number,
      startAfterDoc?: Product,
      orderByField?: string,
      orderByDirection: 'asc' | 'desc' = 'asc'
    ): Observable<{ data: Product[]; lastDoc: Product | null }> {
      return new Observable<{ data: Product[]; lastDoc: Product | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: Product[] = [];
            let lastDoc: Product | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as Product);
              lastDoc = doc.data() as Product; // Get the last document for next page
            });
