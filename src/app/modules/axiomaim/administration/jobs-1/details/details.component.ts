...
export class JobsDetailsComponent implements OnInit, OnDestroy {
     ...
     */
    ngOnInit(): void {
        this.jobs = this._jobsV2Service.jobs();
        this.job = this._jobsV2Service.job();
        // Open the drawer
     ...
     */
    updateItem(): void {        
        this.job = {...this._job.getValue(), ...this.jobForm.getRawValue()};
    ...
    deleteProduct(): void {
        ...
        confirmation.afterClosed().subscribe((result) => {
            ...
            if (result === 'confirmed') {
                ...
                const currentProductIndex = this.jobs.findIndex(
                    (item) => item.id === id
                );
                const nextProductIndex =
                    currentProductIndex +
                    (currentProductIndex === this.jobs.length - 1 ? -1 : 1);
                const nextProductId =
                    this.jobs.length === 1 && this.jobs[0].id === id
                        ? null
                        : this.jobs[nextProductIndex].id;
    ...
    removeAvatar(): void {
        ...
        const avatarFormControl = this.jobForm.get('avatar');
