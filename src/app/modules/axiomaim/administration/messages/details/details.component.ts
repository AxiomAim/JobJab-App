...
export class MessagesDetailsComponent implements OnInit, OnDestroy {
    ...
    private _message: BehaviorSubject<Message | null> = new BehaviorSubject(
        null
    );
    ...
    constructor(
        ...
        private _messagesListComponent: MessagesListComponent,
     ...
     */
    ngOnInit(): void {
        this.messages = this._messagesV2Service.messages();
        this.message = this._messagesV2Service.message();
        // Open the drawer
        this._messagesListComponent.matDrawer.open();
    ...
    closeDrawer(): Promise<MatDrawerToggleResult> {
        return this._messagesListComponent.matDrawer.close();
    }
     ...
     */
    updateItem(): void {        
        this.message = {...this._message.getValue(), ...this.messageForm.getRawValue()};
        console.log('message', this.message);
        // Get the message object
        // const message = this.messageForm.getRawValue();

        // Update the message on the server
        this._messagesV2Service
            .updateItem(this.message)
            .then(() => {
    ...
    deleteProduct(): void {
        ...
        confirmation.afterClosed().subscribe((result) => {
            ...
            if (result === 'confirmed') {
                ...
                // Get the next/previous message's id
                const currentProductIndex = this.messages.findIndex(
                    (item) => item.id === id
                );
                const nextProductIndex =
                    currentProductIndex +
                    (currentProductIndex === this.messages.length - 1 ? -1 : 1);
                const nextProductId =
                    this.messages.length === 1 && this.messages[0].id === id
                        ? null
                        : this.messages[nextProductIndex].id;

                // Delete the message
                this._messagesV2Service
                    .deleteItem(id)
                    .then((isDeleted) => {
                        // Return if the message wasn't deleted...
                        if (!isDeleted) {
                            return;
                        }

                        // Navigate to the next message if available
                        if (nextProductId) {
    ...
    uploadAvatar(event: any): void {
        ...
        // this._messagesV2Service.uploadAvatar(event.target.files[0], 'messages').subscribe({
        //     next: (response: any) => {
        //         this._axiomaimLoadingService.hide();
        //         console.log('uploadAvatar', response)
        //         this.message.avatarPath = response.filePath;
        //         this.message.avatarFile = response.fileName;
        //         this.message.avatarType = response.fileType;
        //         this.message.avatarUrl = response.fileUrl;
        //         this.message.avatar = response.fileUrl;                
    ...
    removeAvatar(): void {
        ...
        const avatarFormControl = this.messageForm.get('avatar');
        ...
        // Update the message
        // this.message.avatar = null;
    }

    onOptionSelected(data: any[]) {
        console.log('onOptionSelected', data);
        // this.message.messageRoles = data;
        this._message.next(this.message);
        // console.log('onOptionSelected', this.message);
        // this.message$.subscribe((resProduct: Product) => {
    ...
    //     const isTagApplied = this.message.tags.find((id) => id === tag.id);

    //     // If the found tag is already applied to the message...
    //     if (isTagApplied) {
