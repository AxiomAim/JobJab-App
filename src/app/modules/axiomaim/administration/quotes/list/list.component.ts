...
import { QuotesAddItemComponent } from '../add-item/add-item.component';
...
export class QuotesListComponent implements OnInit, OnDestroy {
    ...
    private _quotes: BehaviorSubject<Quote[] | null> = new BehaviorSubject(
        null
    );
    ...
    ngOnInit(): void {
        // Get the quotes
        this._quotes.next(this._quotesV2Service.quotes());
        this.quotes$
        ...
        // Get the product
        this._quote.next(this._quotesV2Service.quote());
        this.quote$
            .pipe(takeUntil(this._unsubscribeAll))
            .subscribe((product: Quote) => {
        ...
        this.searchInputControl.valueChanges
            .pipe(
                ...
                switchMap((query) =>
                    // Search
                    this._quotesV2Service.search(query)
                )
            )
            .subscribe((resProducts) => {
                this._quotes.next(resProducts);
            });
