...
export class NewslettersDataService extends BaseDataService<Newsletter> {
    ...
    public getAll(orgId: string): Observable<Newsletter[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    public getItem(id: string): Observable<Newsletter> {
    ...
    public updateItem(data: Partial<Newsletter>): Observable<Newsletter> {
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as Newsletter); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<Newsletter>(this.baseCollection, id, updateData);
      }
    ...
    public createItem(data: Newsletter): Observable<Newsletter> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<Newsletter[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<Newsletter>[]): Observable<Newsletter[]> { 
      return this._firestore.bulkCreate<Newsletter>(this.baseCollection, data as Newsletter[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<Newsletter[]> {
      return this._firestore.getQueryWhereclause<Newsletter>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<Newsletter>[]): Observable<Newsletter[]> { 
      return this._firestore.bulkUpdate<Newsletter>(this.baseCollection, data as Newsletter[]); 
    }
   ...
   */
    public getPaged(
      pageSize: number,
      startAfterDoc?: Newsletter,
      orderByField?: string,
      orderByDirection: 'asc' | 'desc' = 'asc'
    ): Observable<{ data: Newsletter[]; lastDoc: Newsletter | null }> {
      return new Observable<{ data: Newsletter[]; lastDoc: Newsletter | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            ...
            let lastDoc: Newsletter | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as Newsletter);
              lastDoc = doc.data() as Newsletter; // Get the last document for next page
            });
