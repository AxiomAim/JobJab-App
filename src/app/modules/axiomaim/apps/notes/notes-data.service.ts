...
export class NotesDataService extends BaseDataService<Note> {
    ...
    public getAll(orgId: string): Observable<Note[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    
    public getItem(id: string): Observable<Note> {
    ...
    public updateItem(data: Partial<Note>): Observable<Note> {
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as Note); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<Note>(this.baseCollection, id, updateData);
      }
    ...
    public deleteItem(id: string): Observable<Note> {
    ...
    public createItem(data: Note): Observable<Note> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<Note[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<Note>[]): Observable<Note[]> { 
      return this._firestore.bulkCreate<Note>(this.baseCollection, data as Note[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<Note[]> {
      return this._firestore.getQueryWhereclause<Note>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<Note>[]): Observable<Note[]> { 
      return this._firestore.bulkUpdate<Note>(this.baseCollection, data as Note[]); 
    }
   ...
   */
    ...
    ): Observable<{ data: Note[]; lastDoc: Note | null }> {
      return new Observable<{ data: Note[]; lastDoc: Note | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: Note[] = [];
            let lastDoc: Note | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as Note);
              lastDoc = doc.data() as Note; // Get the last document for next page
            });
