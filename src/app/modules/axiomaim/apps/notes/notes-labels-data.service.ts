...
export class NotesLabelsDataService extends BaseDataService<NoteLabel> {
    ...
    public getAll(orgId: string): Observable<NoteLabel[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    
    public getItem(id: string): Observable<NoteLabel> {
    ...
    public updateItem(data: Partial<NoteLabel>): Observable<NoteLabel> {
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as NoteLabel); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<NoteLabel>(this.baseCollection, id, updateData);
      }
    ...
    public createItem(data: NoteLabel): Observable<NoteLabel> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<NoteLabel[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<NoteLabel>[]): Observable<NoteLabel[]> { 
      return this._firestore.bulkCreate<NoteLabel>(this.baseCollection, data as NoteLabel[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<NoteLabel[]> {
      return this._firestore.getQueryWhereclause<NoteLabel>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<NoteLabel>[]): Observable<NoteLabel[]> { 
      return this._firestore.bulkUpdate<NoteLabel>(this.baseCollection, data as NoteLabel[]); 
    }
   ...
   */
    public getPaged(
      pageSize: number,
      startAfterDoc?: NoteLabel,
      orderByField?: string,
      orderByDirection: 'asc' | 'desc' = 'asc'
    ): Observable<{ data: NoteLabel[]; lastDoc: NoteLabel | null }> {
      return new Observable<{ data: NoteLabel[]; lastDoc: NoteLabel | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            ...
            let lastDoc: NoteLabel | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as NoteLabel);
              lastDoc = doc.data() as NoteLabel; // Get the last document for next page
            });
