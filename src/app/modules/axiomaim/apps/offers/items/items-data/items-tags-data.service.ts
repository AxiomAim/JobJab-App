...
export class ItemsTagsDataService extends BaseDataService<ItemsTag> {
    ...
    public getAll(orgId: string): Observable<ItemsTag[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    public getItem(id: string): Observable<ItemsTag> {
    ...
    public updateItem(data: Partial<ItemsTag>): Observable<ItemsTag> {
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as ItemsTag); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<ItemsTag>(this.baseCollection, id, updateData);
      }
    ...
    public createItem(data: ItemsTag): Observable<ItemsTag> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<ItemsTag[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<ItemsTag>[]): Observable<ItemsTag[]> { 
      return this._firestore.bulkCreate<ItemsTag>(this.baseCollection, data as ItemsTag[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<ItemsTag[]> {
      return this._firestore.getQueryWhereclause<ItemsTag>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<ItemsTag>[]): Observable<ItemsTag[]> { 
      return this._firestore.bulkUpdate<ItemsTag>(this.baseCollection, data as ItemsTag[]); 
    }
   ...
   */
    public getPaged(
      pageSize: number,
      startAfterDoc?: ItemsTag,
      orderByField?: string,
      orderByDirection: 'asc' | 'desc' = 'asc'
    ): Observable<{ data: ItemsTag[]; lastDoc: ItemsTag | null }> {
      return new Observable<{ data: ItemsTag[]; lastDoc: ItemsTag | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: ItemsTag[] = [];
            let lastDoc: ItemsTag | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as ItemsTag);
              lastDoc = doc.data() as ItemsTag; // Get the last document for next page
            });
