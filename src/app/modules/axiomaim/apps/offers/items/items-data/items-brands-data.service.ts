...
export class ItemsBrandsDataService extends BaseDataService<ItemsBrand> {
    ...
    public getAll(orgId: string): Observable<ItemsBrand[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    public getItem(id: string): Observable<ItemsBrand> {
    ...
    public updateItem(data: Partial<ItemsBrand>): Observable<ItemsBrand> {
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as ItemsBrand); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<ItemsBrand>(this.baseCollection, id, updateData);
      }
    ...
    public createItem(data: ItemsBrand): Observable<ItemsBrand> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<ItemsBrand[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<ItemsBrand>[]): Observable<ItemsBrand[]> { 
      return this._firestore.bulkCreate<ItemsBrand>(this.baseCollection, data as ItemsBrand[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<ItemsBrand[]> {
      return this._firestore.getQueryWhereclause<ItemsBrand>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<ItemsBrand>[]): Observable<ItemsBrand[]> { 
      return this._firestore.bulkUpdate<ItemsBrand>(this.baseCollection, data as ItemsBrand[]); 
    }
   ...
   */
    public getPaged(
      pageSize: number,
      startAfterDoc?: ItemsBrand,
      orderByField?: string,
      orderByDirection: 'asc' | 'desc' = 'asc'
    ): Observable<{ data: ItemsBrand[]; lastDoc: ItemsBrand | null }> {
      return new Observable<{ data: ItemsBrand[]; lastDoc: ItemsBrand | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: ItemsBrand[] = [];
            let lastDoc: ItemsBrand | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as ItemsBrand);
              lastDoc = doc.data() as ItemsBrand; // Get the last document for next page
            });
