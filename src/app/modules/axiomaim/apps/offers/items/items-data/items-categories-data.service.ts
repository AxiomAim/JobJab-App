...
export class ItemsCategoriesDataService extends BaseDataService<ItemsCategory> {
    ...
    public getAll(orgId: string): Observable<ItemsCategory[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    public getItem(id: string): Observable<ItemsCategory> {
    ...
    public updateItem(data: Partial<ItemsCategory>): Observable<ItemsCategory> {
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as ItemsCategory); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<ItemsCategory>(this.baseCollection, id, updateData);
      }
    ...
    public createItem(data: ItemsCategory): Observable<ItemsCategory> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<ItemsCategory[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<ItemsCategory>[]): Observable<ItemsCategory[]> { 
      return this._firestore.bulkCreate<ItemsCategory>(this.baseCollection, data as ItemsCategory[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<ItemsCategory[]> {
      return this._firestore.getQueryWhereclause<ItemsCategory>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<ItemsCategory>[]): Observable<ItemsCategory[]> { 
      return this._firestore.bulkUpdate<ItemsCategory>(this.baseCollection, data as ItemsCategory[]); 
    }
   ...
   */
    public getPaged(
      pageSize: number,
      startAfterDoc?: ItemsCategory,
      orderByField?: string,
      orderByDirection: 'asc' | 'desc' = 'asc'
    ): Observable<{ data: ItemsCategory[]; lastDoc: ItemsCategory | null }> {
      return new Observable<{ data: ItemsCategory[]; lastDoc: ItemsCategory | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: ItemsCategory[] = [];
            let lastDoc: ItemsCategory | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as ItemsCategory);
              lastDoc = doc.data() as ItemsCategory; // Get the last document for next page
            });
