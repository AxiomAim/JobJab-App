...
export class ItemsVendorsDataService extends BaseDataService<ItemsVendor> {
    ...
    public getAll(orgId: string): Observable<ItemsVendor[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    public getItem(id: string): Observable<ItemsVendor> {
    ...
    public updateItem(data: Partial<ItemsVendor>): Observable<ItemsVendor> {
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as ItemsVendor); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<ItemsVendor>(this.baseCollection, id, updateData);
      }
    ...
    public createItem(data: ItemsVendor): Observable<ItemsVendor> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<ItemsVendor[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<ItemsVendor>[]): Observable<ItemsVendor[]> { 
      return this._firestore.bulkCreate<ItemsVendor>(this.baseCollection, data as ItemsVendor[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<ItemsVendor[]> {
      return this._firestore.getQueryWhereclause<ItemsVendor>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<ItemsVendor>[]): Observable<ItemsVendor[]> { 
      return this._firestore.bulkUpdate<ItemsVendor>(this.baseCollection, data as ItemsVendor[]); 
    }
   ...
   */
    public getPaged(
      pageSize: number,
      startAfterDoc?: ItemsVendor,
      orderByField?: string,
      orderByDirection: 'asc' | 'desc' = 'asc'
    ): Observable<{ data: ItemsVendor[]; lastDoc: ItemsVendor | null }> {
      return new Observable<{ data: ItemsVendor[]; lastDoc: ItemsVendor | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: ItemsVendor[] = [];
            let lastDoc: ItemsVendor | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as ItemsVendor);
              lastDoc = doc.data() as ItemsVendor; // Get the last document for next page
            });
