...
export class TagsDataService extends BaseDataService<Tag> {
    ...
    public getAll(orgId: string): Observable<Tag[]> {
    ...
    public updateItem(data: Partial<Tag>): Observable<Tag> {
        ...
        if (Object.keys(updateData).length === 0) {
          return of(data as Tag); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<Tag>(this.baseCollection, id, updateData);
      }
    ...
    public bulkCreate(data: Partial<Tag>[]): Observable<Tag[]> { 
      return this._firestore.bulkCreate<Tag>(this.baseCollection, data as Tag[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<Tag[]> {
      return this._firestore.getQueryWhereclause<Tag>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<Tag>[]): Observable<Tag[]> { 
      return this._firestore.bulkUpdate<Tag>(this.baseCollection, data as Tag[]); 
    }
   ...
   */
    ...
    ): Observable<{ data: Tag[]; lastDoc: Tag | null }> {
      return new Observable<{ data: Tag[]; lastDoc: Tag | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: Tag[] = [];
            let lastDoc: Tag | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as Tag);
              lastDoc = doc.data() as Tag; // Get the last document for next page
            });
