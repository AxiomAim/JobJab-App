...
export class TasksDataService extends BaseDataService<Task> {
    ...
    public getAll(orgId: string): Observable<Task[]> {
        return  this._firestore.getAll(orgId, this.baseCollection);
    }
    public getItem(id: string): Observable<Task> {
    ...
    public updateItem(data: Partial<Task>): Observable<Task> {
        // Assuming 'id' is the primary key
        const { id, ...updateData } = data; 
        // Only update if there are actual changes
        if (Object.keys(updateData).length === 0) {
          return of(data as Task); // Or throw an error if this shouldn't happen
        }
      
        return this._firestore.updateItem<Task>(this.baseCollection, id, updateData);
      }
    ...
    public deleteItem(id: string): Observable<Task> {
    ...
    public createItem(data: Task): Observable<Task> {
    ...
    public getQuery(fieldName: string, operator: WhereFilterOp, value: string): Observable<Task[]> {
        return this._firestore.getQuery(this.baseCollection, fieldName, operator, value);
    }

    public bulkCreate(data: Partial<Task>[]): Observable<Task[]> { 
      return this._firestore.bulkCreate<Task>(this.baseCollection, data as Task[]); 
    }

    public getQueryWhereclause(queries: FirestoreQuery[]): Observable<Task[]> {
      return this._firestore.getQueryWhereclause<Task>(this.baseCollection, queries);
  }


    public bulkUpdate(data: Partial<Task>[]): Observable<Task[]> { 
      return this._firestore.bulkUpdate<Task>(this.baseCollection, data as Task[]); 
    }
   ...
   */
    ...
    ): Observable<{ data: Task[]; lastDoc: Task | null }> {
      return new Observable<{ data: Task[]; lastDoc: Task | null }>((observer) => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            const data: Task[] = [];
            let lastDoc: Task | null = null;
  
            querySnapshot.forEach((doc) => {
              data.push(doc.data() as Task);
              lastDoc = doc.data() as Task; // Get the last document for next page
            });
